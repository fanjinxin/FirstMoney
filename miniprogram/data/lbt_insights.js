 ...
 * 移植自 src/data/lbt_insights.ts
 */
const LBT_LEVEL_LABELS = { low: '偏低', moderate: '适中', high: '偏高' };
...
const LBT_DIMENSION_INSIGHTS = {
  depend: '情感依赖反映你对伴侣态度与回应的敏感度。适度的在意能增进亲密；过高可能导致焦虑、情绪起伏大。',
  prioritize: '优先倾斜反映你为关系付出的程度。适度付出是爱的体现；过高可能导致忽略朋友、改变计划、失去自我。',
  balance: '理性平衡反映你在恋爱与事业/目标间的协调能力。能力强者可兼顾亲密与独立；能力弱者易在恋爱中倾斜过度。',
};

const LBT_RELATIONSHIP_TIPS = {
  rational: ['保持理性与边界，同时适度表达情感与在意', '若伴侣更需要亲密感，可适当增加陪伴与表达', '避免让伴侣感到疏离或不被需要'],
  balanced: ['继续保持觉察，在投入与边界之间灵活调整', '热恋期注意保持朋友圈与个人兴趣', '与伴侣沟通彼此对独立空间的需求'],
  invested: ['培养 1–2 个独立于关系的兴趣或爱好', '维持与朋友的定期联系，不因恋爱忽略社交', '设定个人目标，即使恋爱也保留成长空间'],
  obsessive: ['优先练习情绪独立，不把自我价值完全系于对方态度', '若焦虑、空虚感持续，建议寻求专业心理咨询', '与伴侣坦诚沟通自己的不安，共同建立健康的边界'],
};

function getLBTProfileKey(levelKey, dimensionScores) {
  if (levelKey === 'low') return 'rational';
  if (levelKey === 'high') {
