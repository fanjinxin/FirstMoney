/**
 * HIT 霍兰德职业兴趣测试 - 移植自 src/data/hit.ts
 * 90 题，6 维度（RIASEC），每题 是/否，选「是」得 1 分
 */
const HIT_DIMENSIONS = [
...
const hitTest = {
  ...
  title: 'HIT 霍兰德职业兴趣测试',
  subtitle: '发现你的职业兴趣类型',
  description: '霍兰德将职业兴趣分为六型：现实型(R)、研究型(I)、艺术型(A)、社会型(S)、企业型(E)、常规型(C)。通过 90 道题目，评估你在各类型的兴趣强度，生成霍兰德代码（如 RIA、SEC），为职业选择与生涯规划提供参考。',
  instructions: ['请根据真实兴趣作答，不必考虑能力或外界期待。', '每题二选一：是 / 否。', '选择「是」表示该描述符合你的兴趣倾向。', '回答时间无限制，凭第一感受作答即可。'],
  format: 'choice',
  options: ['是', '否'],
  questions: hitQuestions.map(q => ({ id: String(q.id), text: q.text, dimension: q.dimension, options: ['是', '否'] })),
};

module.exports = { hitTest, hitQuestions, HIT_DIMENSIONS };
