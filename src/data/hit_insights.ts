/**
 * HIT 霍兰德职业兴趣 - 维度解读、霍兰德代码说明与职业推荐
 */
import type { HITDimensionId } from './hit'

/** 霍兰德代码简短说明（用于图标旁） */
export const HOLLAND_CODE_INTRO = {
  what: '六型 RIASEC 按得分取前三组成代码，用于匹配职业。',
}

/** 六型相邻关系（用于分析代码一致性） */
export const HOLLAND_ADJACENT: Record<string, string[]> = {
  R: ['I', 'C'],
  I: ['R', 'A'],
  A: ['I', 'S'],
  S: ['A', 'E'],
...
/** 判断代码一致性：相邻多=高一致，对角多=低一致 */
export function getCodeConsistency(topThree: string[]): { level: string; short: string } {
  const ids = topThree.slice(0, 3)
  let adjacentCount = 0
...
/** 简短一句话（用于卡片主显示） */
export const HIT_DIMENSION_SHORT: Record<HITDimensionId, string> = {
...
export const HIT_DIMENSION_INSIGHTS: Record<HITDimensionId, string> = {
...
export const HOLLAND_CODE_EXAMPLES: Record<string, string> = {
  RIA: '工程师、机械设计师、地质学家',
  RIC: '机械工程师、测量员、制图员',
  RIE: '建筑师、土木工程师、飞行员',
  RSE: '警察、消防员、保安',
