...
export type AnimalDimension = 'E' | 'A' | 'C' | 'N' | 'O'; // Big Five: Extraversion, Agreeableness, Conscientiousness, Neuroticism, Openness
...
export interface AnimalQuestion {
  ...
  options: {
    label: string;
    score: Partial<Record<AnimalDimension, number>>;
  }[];
}
...
export interface AnimalArchetype {
  ...
  description: string;
  traits: string[];
  weaknesses: string[]; // Added weaknesses
  dimensions: Record<AnimalDimension, number>; // 0-10 scale
}
...
export const animalArchetypes: AnimalArchetype[] = [
  {
    id: 'lion',
    name: '狮子',
    emoji: '🦁',
